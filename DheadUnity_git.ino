...
void commandfromHMD(const std_msgs::Float32MultiArray& msg) 
{
  ...
  if (inv_roll == false) oroll = map(inputVector[5], 0, 300, maxValue, 0);
  else oroll = map(inputVector[5], 0, 300, 0, maxValue);
  if (inv_pitch == false) opitch = map(inputVector[3], 0, 300, maxValue, 0);
  else opitch = map(inputVector[3], 0, 300, 0, maxValue);
  if (inv_yaw == false) oyaw = map(inputVector[4], 0, 300, maxValue, 0);
  else oyaw = map(inputVector[4], 0, 300, 0, maxValue);
  stringComplete = true;
}
